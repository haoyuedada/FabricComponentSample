...
const metro_transformer = require('metro-react-native-babel-transformer/src/index.js');
...
exportObject.transform = function (conf) {
    // init();
    console.log("chy transformer")
    if (exportObject._miot_building && !exportObject._miot_building_plugin) {
        if (!conf.plugins) {
            conf.plugins = [];
        }
